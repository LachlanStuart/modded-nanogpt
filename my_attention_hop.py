...
def flex_attention_fake_impl(query: torch.Tensor, value: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
def create_fw_bw_graph(
    ...
    with suspend_functionalization(), disable_functional_mode():
        with disable_proxy_modes_tracing():
            ...
            def _from_fun(t: Union[Tensor, torch.SymInt, int]) -> Union[Tensor, torch.SymInt, int]:
        ...
        def joint_f(
            ...
            *other_buffers: Tuple[Tensor, ...],
            ...
            def fw_with_masks(*args: Tuple[Tensor, ...]) -> Tuple[Tuple[Tensor], Tuple[bool]]:
...
def flex_attention_backward_fake_tensor_mode(
    ...
    with mode:
        ...
        grad_score_mod_captured = tuple(
            [
                torch.empty_like(buffer) if isinstance(buffer, torch.Tensor) and buffer.requires_grad else None
