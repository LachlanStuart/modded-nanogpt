...
from .my_attention_hop import flex_attention as flex_attention_hop
...
def _get_mod_type(fn: Callable) -> _ModificationType:
    ...
    num_positional_args = sum(
        1 for param in inspect.signature(fn).parameters.values() if param.default == inspect.Parameter.empty
    )
...
def create_block_mask(
    ...
    assert mod_type == _ModificationType.MASK_MOD, f"create-block_mask requires a mask_mod function! Got {mask_mod}"
    if B is None:
        B = 1
    if H is None:
        H = 1
...
def _nested_mod_func_adapter(
    ...
    def _build_seq_idx(offsets, total_length):
        range_tensor = torch.arange(total_length, device=offsets.device, dtype=torch.int32)
...
def create_nested_block_mask(
    ...
    if q_nt.device != kv_nt.device:
        raise ValueError("create_nested_block_mask(): Expected q_nt and kv_nt to be on the same device")
            ...
            with _temp_remove_pre_dispatch_torch_function_mode():
                with _temp_remove_metadata_torch_function_mode() as metadata_mode:
                    if metadata_mode:
                        ...
                        backend = "eager"
                    out, lse = torch.compile(_flex_attention_hop_wrapper, backend=backend, fullgraph=True)(
                        query,
                        key,
                        value,
                        score_mod,
